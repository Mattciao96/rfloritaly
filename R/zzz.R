...
.onAttach <- function(libname,pkgname) {
  ...
  if(local_version != remote_version) {
    packageStartupMessage("Please run devtools::install_github('gibedini/rfloritaly') to install the latest version (", remote_version, ").")
  } else {
...
get_local_version <- function() {
  ...
  utils::data("ckl_version",package = "rfloritaly", envir = environment())
